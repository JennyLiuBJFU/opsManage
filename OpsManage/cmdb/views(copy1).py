...
def asset_1(request):
    ...
    myNet = int(request.GET.get('myOrg', "0"))
    ...
    if (myType == 0) and (myOrg == 0) and (myNet == 0):
        asset_find = Asset.objects.all()

    elif (myType != 0) and (myOrg == 0) and (myNet == 0):
        asset_find = Asset.objects.filter(asset_type=myType)

    elif (myType == 0) and (myOrg != 0) and (myNet == 0):
        asset_find = Asset.objects.filter(organization=myOrg)

    elif (myType != 0) and (myOrg != 0):
        # asset_find = Asset.objects.filter(
        #     Q(asset_type=myType) |
        #     Q(organization=myOrg))
        asset_find = Asset.objects.filter(asset_type=myType).filter(organization=myOrg)
    print(asset_find)
    ...
    if keyword:
        asset_find = Asset.objects.filter(
            Q(asset_name__contains=keyword) |
            Q(asset_type=keyword) |
            Q(asset_no__contains=keyword) |
            Q(organization=keyword) |
            Q(manage_ip__contains=keyword) |
            Q(idc=keyword) |
            Q(supplier=keyword) |
            Q(model=keyword)
        )
...
def filter(request):
    typeID = request.GET.get('myType','0')
    orgID = request.GET.get('myOrg','0')
...
def filter(request):
    ...
    if (typeID == "0" and orgID == "0"):
        ...
        Assets = Asset.objects.filter(asset_type=request.GET['myType'])

    elif (typeID == "0" and orgID != "0"):
        Assets = Asset.objects.filter(organization=request.GET['myOrg'])

    else:
        Assets = Asset.objects.filter(organization=request.GET['myOrg']).filter(organization=request.GET['myOrg'])
    ...
    print(context)
    # return render(request, 'cmdb/ServerManage/index.html', context)
    return render(request, 'cmdb/filter.html', context)
...
def filter(request):
    print(request)
    asset_type = request.POST.get('ftype')
    if asset_type is None:
        ASSET = Asset.objects.all()
    elif asset_type:
        ASSET = Asset.objects.filter(asset_type=asset_type)
    print("================")
    print(asset_type)
    print(ASSET)
    print("================")
    ...
    return HttpResponse("cmdb/",context)
"""
