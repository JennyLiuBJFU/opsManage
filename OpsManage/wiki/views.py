...
def article_add(request):
    ...
    #将现有TAG catgory数据传给前端
    if request.method == "GET":
        tagList = Tag.objects.all()
        categoryList = Category.objects.all()
        return render(request, 'wiki/wiki_post.html',
                      {"user": request.user, "tagList": tagList, "categoryList": categoryList})
    ...
    #根据前端提交的数据创建catgory对象
    try:
        category = Category.objects.get(id=category)
    except Exception as e:
        # logger.warn(msg="获取分类失败: {e}".format(ex=str(e)))
        return JsonResponse({'msg': "获取分类失败: {ex}".format(ex=str(e)), "code": 500, 'data': []})

    # 根据前端提交的数据创建article对象
    try:
        article = Post.objects.create(title=title, content=content, category=category,
                                      author=User.objects.get(username=request.user))
    except Exception as e:
        # logger.warn(msg="创建文章失败: {ex}".format(ex=str(e)))
        return JsonResponse({'msg': "创建文章失败: {ex}".format(ex=str(e)), "code": 500, 'data': []})
    ...
    try:
        ...
        pass
        # logger.warn(msg="创建文章标签失败: {ex}".format(ex=str(e)))
    ...
    if request.method == 'POST':
        ...
        if f:
            try:
                ...
                pass
                # logger.error(msg="上传图片失败: {ex}".format(ex=e))
...
def article_edit(request, pid):
    try:
        ...
        # logger.error(msg="文章不存在: {ex}".format(ex=e))
        return render(request, 'wiki/wiki_edit.html', {"user": request.user, "errorInfo": e})
    ...
    if request.method == "GET":
        ...
        return render(request, 'wiki/wiki_edit.html',
                      {"user": request.user, "tagList": tagList, "categoryList": categoryList, "article": article})
        ...
        try:
            ...
            return JsonResponse({'msg': "更新文章失败: {ex}".format(ex=str(e)), "code": 500, 'data': []})
        try:
            newTagsList = []
            for tg in tags:
...
def article_index(request):
    ...
    dateList = Post.objects.raw(
        'SELECT id,DATE_FORMAT( created_time, "%%Y/%%m" ) as cdate , COUNT(*) '
        'as count FROM opsmanage_wiki_post GROUP BY DATE_FORMAT( created_time, "%%Y/%%m" );'
    )
    return render(request, 'wiki/wiki_base.html',
                  {"user": request.user, "tagList": tagList, "categoryList": categoryList, "postList": postList,
                   "dateList": dateList})
...
def article_show(request, pid):
    ...
    if request.method == "GET":
        ...
        dateList = Post.objects.raw(
            'SELECT id,DATE_FORMAT( created_time, "%%Y/%%m" ) as cdate , COUNT(*) '
            'as count FROM opsmanage_wiki_post GROUP BY DATE_FORMAT( created_time, "%%Y/%%m" );')
        return render(request, 'wiki/wiki_show.html',
                      {"user": request.user, "tagList": tagList, "categoryList": categoryList, "article": article,
...
def article_archive(request, month):
    try:
        ...
        return render(request, 'wiki/wiki_base.html',
                      {"user": request.user, "errorInfo": "归档时间不存在: {e}".format(ex=e)})
    categoryList = []
    for ds in Category.objects.all():
        ds.post_count = Post.objects.filter(category=ds).count()
        categoryList.append(ds)
    postList = Post.objects.raw(
        '''SELECT * FROM opsmanage_wiki_post WHERE created_time>='{startime}' AND created_time<='{endtime}' ORDER BY id DESC;'''.format(
            startime=str(archiveDate[0]) + ' 00:00:00', endtime=str(archiveDate[1]) + ' 23:59:59'))
    dateList = Post.objects.raw(
        'SELECT id,DATE_FORMAT( created_time, "%%Y/%%m" ) as cdate , COUNT(*) as count FROM '
        'opsmanage_wiki_post GROUP BY DATE_FORMAT( created_time, "%%Y/%%m" );')
    tagList = Tag.objects.all()
    return render(request, 'wiki/wiki_base.html',
                  {"user": request.user, "postList": postList, "dateList": dateList, "tagList": tagList,
...
def article_category(request, pid):
    try:
        ...
        # logger.warn(msg="分类不存在: {ex}".format(ex=e))
        return render(request, 'wiki/wiki_base.html', {"user": request.user, "errorInfo": "分类不存在: {ex}".format(ex=e)})
    categoryList = []
    for ds in Category.objects.all():
        ds.post_count = Post.objects.filter(category=ds).count()
        categoryList.append(ds)
...
def article_tag(request, pid):
    try:
        ...
        # logger.warn(msg="标签不存在: {ex}".format(ex=e))
        return render(request, 'wiki/wiki_base.html', {"user": request.user, "errorInfo": "标签不存在: {ex}".format(ex=e)})
    categoryList = []
    for ds in Category.objects.all():
        ds.post_count = Post.objects.filter(category=ds).count()
        categoryList.append(ds)
